...
extension vector_float4 {
    ...
    init(position: Vector) {
        self.init(x: position.x, y: position.y, z: position.z, w: 1)
    }
    
    init(velocity: Vector) {
        self.init(x: velocity.x, y: velocity.y, z: velocity.z, w: 0)
    }
    
    mutating func transform(matrix: matrix_float4x4) {
...
extension matrix_float4x4 {
    ...
    func translatedBy(displacement: Vector) -> matrix_float4x4 {
        let translateMatrix = matrix_float4x4(translationX: displacement.x, y: displacement.y, z: displacement.z)
        return matrix_multiply(self, translateMatrix)
    }
    ...
    func rotatedBy(rotationAngle angle: Float,
                   axis: Vector) -> matrix_float4x4 {
        let rotationMatrix = matrix_float4x4(rotationAngle: angle,
                                             x: axis.x, y: axis.y, z: axis.z)
        return matrix_multiply(self, rotationMatrix)
    }
    
    init(fov: Float, aspect: Float, nearZ: Float, farZ: Float) {
        let y = 1 / tan(fov * 0.5)
        let x = y / aspect
